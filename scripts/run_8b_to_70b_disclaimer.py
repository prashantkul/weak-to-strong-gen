"""
8B→70B Disclaimer Variant Experiment
Test if warning helps 70B overcome weak label degradation across all K values
"""
...
async def main():
    print("\n" + "="*70)
    print("8B→70B DISCLAIMER EXPERIMENT")
    print("Can warning help 70B overcome label noise degradation?")
    print("="*70)
    ...
    for k in K_VALUES:
        ...
        if k == 0:
            ...
            print(f"\nStrong + Weak + DISCLAIMER (70B, K={k})")
    ...
    if avg_change > 0.03:
        print(f"\n✓ SIGNIFICANT IMPROVEMENT: Disclaimer helped 70B!")
        print(f"  Average PGR change: {avg_change:+.3f}")
        print(f"  Largest improvement at K={max_change_k}: {max_change:+.3f}")
        print(f"  Metacognitive prompting helps mid-sized models overcome noise")
        print(f"\n  Key insight: 70B benefits from disclaimer more than 405B")
    elif avg_change < -0.02:
        print(f"\n✗ DEGRADATION: Disclaimer hurt 70B performance")
        print(f"  Average PGR change: {avg_change:+.3f}")
        print(f"  Over-cautioning may have reduced model confidence")
    else:
        print(f"\n→ MINIMAL EFFECT: Disclaimer had small impact on 70B")
        print(f"  Average PGR change: {avg_change:+.3f}")
        print(f"  Disclaimer doesn't significantly help or hurt 70B")
    ...
    ax1.set_title('70B: Baseline vs Disclaimer PGR', fontsize=14, fontweight='bold')
    ...
    print("\n" + "="*70)
    print("✓ 8B→70B DISCLAIMER EXPERIMENT COMPLETE")
    print("="*70)

    print(f"\nComparison summary:")
    print(f"  Average PGR change: {avg_change:+.3f}")
    print(f"  Largest improvement at K={max_change_k}: {max_change:+.3f}")
    print(f"\nNext: Compare 405B vs 70B disclaimer effectiveness")
