"""
Chain-of-Thought (CoT) Variant Experiment - 405B
Test if explicit reasoning helps 405B overcome weak label noise
"""
...
async def main():
    print("\n" + "="*70)
    print("8B→405B CHAIN-OF-THOUGHT EXPERIMENT")
    print("Does explicit reasoning help overcome weak label noise?")
    print("="*70)
    ...
    if avg_change > 0.02:
        print(f"\n✓ IMPROVEMENT: Chain-of-Thought helped 405B!")
        print(f"  Average PGR change: {avg_change:+.3f}")
        print(f"  Largest improvement at K={max_change_k}: {max_change:+.3f}")
        print(f"  Explicit reasoning provides consistent benefit")
    elif avg_change < -0.02:
        print(f"\n✗ DEGRADATION: Chain-of-Thought hurt 405B performance")
        print(f"  Average PGR change: {avg_change:+.3f}")
        print(f"  Reasoning overhead may reduce performance")
    else:
        print(f"\n→ MINIMAL EFFECT: Chain-of-Thought had little impact on 405B")
        print(f"  Average PGR change: {avg_change:+.3f}")
        print(f"  405B already highly robust without explicit reasoning")
        print(f"  Model can filter noise without step-by-step prompting")
    ...
    ax1.set_title('405B: Baseline vs CoT PGR', fontsize=14, fontweight='bold')
    ...
    print("\n" + "="*70)
    print("✓ 405B COT EXPERIMENT COMPLETE")
    print("="*70)

    print(f"\nKey Finding:")
    print(f"  Average PGR change: {avg_change:+.3f}")
    print(f"  Largest improvement at K={max_change_k}: {max_change:+.3f}")
    print(f"\nNext: Run 70B cot for comparison")
